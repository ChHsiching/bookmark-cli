...
import { looksLikeBackup, parseBackupJson, parseNetscapeHtml } from '../importer.js';
 ...
 */
function importHtml(parsed, store, now) {
    const byUrl = new Map();
    let skipped = 0;
    let nonWeb = 0;
 ...
 */
function restoreBackup(backup, store) {
...
export async function runImport(file, deps = {}) {
    ...
    try {
        ...
        throw new CliError(`Cannot read import file ${file}: ${err.message}`);
    }
    if (text.charCodeAt(0) === 0xfeff)
        text = text.slice(1); // tolerate a UTF-8 BOM
    const store = deps.store ?? Store.load();
    const now = deps.now ?? (() => new Date());
    const json = tryParseJson(text);
    if (json.ok) {
        if (!looksLikeBackup(json.value)) {
            throw new CliError(`${file} is valid JSON but not a bookmark-cli backup ` +
        ...
        throw new CliError(`Unrecognized import format: ${file}. ` +
