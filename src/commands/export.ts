...
import { CliError, ExportOptions, StoreData } from '../types.js';
...
const SERIALIZERS: Record<ExportFormat, (data: StoreData) => string> = {
  md: toMarkdown,
  html: toNetscapeHtml,
  json: toJson,
};
 ...
 */
export async function runExport(opts: ExportOptions, deps: ExportDeps = {}): Promise<void> {
  const format = opts.format ?? 'md';
  if (!isExportFormat(format)) {
    throw new CliError(
      `Invalid format "${format}". Supported formats: ${FORMATS.join(', ')}.`,
    );
  }
