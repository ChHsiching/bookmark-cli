 ...
 */
export const FIELD_WEIGHTS = { title: 100, tag: 60, url: 40, note: 20 } as const;
 ...
 *   "digitally...").
 */
function subsequenceQuality(query: string, text: string): number | null {
  ...
  const startPenalty =
    Math.min(positions[0], MAX_PENALIZED_START) * START_PENALTY_PER_CHAR;
...
export function scoreBookmark(query: string, bookmark: Bookmark): number {
  ...
  let bestTagQuality: number | null = null;
